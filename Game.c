...
void obsluzDotyk(Game *g,uint16_t x, uint16_t y)
{
		...
		&& y > AllyYCoor && y < AllyYCoor + AllyHeight)
	{
		...
		if(wybranyKwadracikPlansza[0] && wybranyKwadracikPlansza[1]){
			fixedX = ((wybranyKwadracikPlansza[0] - AllyXCoor) / 13) % 10;
			fixedY = ((wybranyKwadracikPlansza[1] - AllyYCoor) / 13) % 10;
		...
		fixedX = ((x-AllyXCoor) / 13) % 10;
		fixedY = ((y-AllyYCoor) / 13) % 10;
		DrawSquare(5 + 15 * fixedX, 5 + 15 * fixedY,LCDBlack,LCDGreen);
		
		
	}
	else if(x >= StatkiX && x <= StatkiX + StatkiW && y >= JednoY && y <= PiecioY + StatkiW)
	{
		...
		if(y >= JednoY && y <= JednoY + StatkiH){
				wybranyKwadracikStatek[1] = 22;
		}
		else if(y >= DwuY && y <= DwuY + StatkiH){
				wybranyKwadracikStatek[1] = 42;
		}
		else if(y >= TrzyY && y <= TrzyY + StatkiH){
				wybranyKwadracikStatek[1] = 62;
		}
		else if(y >= CzteroY && y <= CzteroY + StatkiH){
				wybranyKwadracikStatek[1] = 82;
		}
		...
		&& x >= PrzyciskLewoX && x <= PrzyciskDolX + PrzyciskW)
	{
			...
			if(wybranyKwadracikOpcje[0] && wybranyKwadracikOpcje[1]){
				if(wybranyKwadracikOpcje[0] < 180)DrawButton("<",165,285,LCDBlack,LCDWhite);
				else if(wybranyKwadracikOpcje[0] < 190) DrawButton("^",180,285,LCDBlack,LCDWhite);
				else if(wybranyKwadracikOpcje[0] < 200) DrawButton(">",195,285,LCDBlack,LCDWhite);
				else DrawButton("v",210,285,LCDBlack,LCDWhite);	
			}
			
			wybranyKwadracikOpcje[0] = x;
			wybranyKwadracikOpcje[1] = y;
			if(wybranyKwadracikOpcje[0] < 180)DrawButton("<",165,285,LCDBlack,LCDPastelBlue);
			else if(wybranyKwadracikOpcje[0] < 190) DrawButton("^",180,285,LCDBlack,LCDPastelBlue);
			else if(wybranyKwadracikOpcje[0] < 200) DrawButton(">",195,285,LCDBlack,LCDPastelBlue);
			else DrawButton("v",210,285,LCDBlack,LCDPastelBlue);	
	...
	if(wybranyKwadracikOpcje[0] && wybranyKwadracikOpcje[1] && wybranyKwadracikPlansza[0] && wybranyKwadracikPlansza[1]
		&& wybranyKwadracikStatek[0] && wybranyKwadracikStatek[1])
		{
			...
			if(wybranyKwadracikOpcje[0] < 180)
			{
				DrawButton("<",165,285,LCDBlack,LCDWhite);
				zwrot = '<';
			}
			else if(wybranyKwadracikOpcje[0] < 190)
			{
				DrawButton("^",180,285,LCDBlack,LCDWhite);
				zwrot = '^';
			}
			else if(wybranyKwadracikOpcje[0] < 200) 
			{
				DrawButton(">",195,285,LCDBlack,LCDWhite);
				zwrot = '>';
			}
			else 
			{
				DrawButton("v",210,285,LCDBlack,LCDWhite);
				zwrot = 'v';
			}
			
			DrawBox(wybranyKwadracikStatek[0], wybranyKwadracikStatek[1],15,15,LCDBlack,LCDWhite);
			if(wybranyKwadracikStatek[1]  == 22) maszty = 1;
			else if(wybranyKwadracikStatek[1]  == 42) maszty = 2;
			else if(wybranyKwadracikStatek[1]  == 62) maszty = 3;
			else if(wybranyKwadracikStatek[1]  == 82) maszty = 4;
			
			uint16_t fixedX = ((wybranyKwadracikPlansza[0] - AllyXCoor) / 13) % 10;
			uint16_t fixedY = ((wybranyKwadracikPlansza[1] - AllyYCoor) / 13) % 10;
			DrawSquare(5 + 15 * fixedX, 5 + 15 * fixedY,LCDBlack,LCDWhite);
...
void init(Game* g)
{
	...
	DrawButton("Ustaw",170,260,LCDBlack,LCDWhite);
	DrawButton("<",165,285,LCDBlack,LCDWhite);
	DrawButton("^",180,285,LCDBlack,LCDWhite);
	DrawButton(">",195,285,LCDBlack,LCDWhite);
	DrawButton("v",210,285,LCDBlack,LCDWhite);
	...
	DrawBox(220,22,15,15,LCDBlack,LCDWhite);
	DrawBox(220,42,15,15,LCDBlack,LCDWhite);
	DrawBox(220,62,15,15,LCDBlack,LCDWhite);
	DrawBox(220,82,15,15,LCDBlack,LCDWhite);
...
void handleTurn(Game* g, uint16_t *x, uint16_t* y, uint16_t *fixedXQ, uint16_t* fixedYQ)
{
	...
	if(!empty(&stack))
	{
				...
				&& *y > EnemyYCoor && *y < EnemyYCoor + EnemyHeight)
		{
			...
			if(wybranyKwadracikPlansza[0] && wybranyKwadracikPlansza[1]){
				fixedX = ((wybranyKwadracikPlansza[0] - EnemyXCoor) / 13) % 10;
				fixedY = ((wybranyKwadracikPlansza[1] - EnemyYCoor) / 13) % 10;
			...
			wybranyKwadracikPlansza[0] = *x;
			wybranyKwadracikPlansza[1] = *y;
			fixedX = ((*x-EnemyXCoor) / 13) % 10;
			fixedY = ((*y-EnemyYCoor) / 13) % 10;
